...
(() => {
  ...
  function initLeafColors() {
    ...
    LEAF_COLORS = ["--leaf1", "--leaf2", "--leaf3"]
      .map((k) => cs.getPropertyValue(k).trim())
      .filter(Boolean);
    if (LEAF_COLORS.length === 0)
      LEAF_COLORS = ["#2ecc71", "#27ae60", "#1faa59"];
  }
  ...
  function drawWind(now) {
    const elapsed = now - start,
      t = Math.min(1, elapsed / CFG.DURATION),
      speed = gust(t);
  ...
  function makeLeaf(isSettled = false, y0 = -40, progress = 0) {
    ...
    return {
      ...
      s: size,
      rot: R(0, Math.PI * 2),
      rotSpd: R(-0.03, 0.03) * (STORM ? 1.5 : 1),
      swayPh: R(0, Math.PI * 2),
      col,
      settled: isSettled,
    };
  }
  ...
  function startPilePhase() {
    ...
    settled = Array.from({ length: 18 }, () =>
      makeLeaf(true, h - R(10, 40), 0),
    );
    const startCount = STORM ? PILE.FALLERS * 1.4 : PILE.FALLERS;
    fallers = Array.from({ length: Math.floor(startCount) }, () =>
  ...
  function drawPile(now) {
    ...
    for (let i = fallers.length - 1; i >= 0; i--) {
      const L = fallers[i];
      const sp =
        PILE.BASE_SPEED * (STORM ? 2.2 : 1) * (0.8 + Math.random() * 0.6) * dpr;
      L.y += sp;
      L.x +=
        Math.sin(L.swayPh + now * 0.002) *
        (PILE.SWAY * dpr) *
        (STORM ? 1.4 : 1);
      L.rot += L.rotSpd;
    ...
    if (pileHeight < h * 0.995) {
      if (fallers.length < (STORM ? PILE.FALLERS * 1.1 : PILE.FALLERS * 0.6)) {
        ...
        const addCount = Math.max(
          baseCnt,
          Math.floor(baseCnt + progress * slope),
        );
        fallers.push(
          ...Array.from({ length: addCount }, () =>
            makeLeaf(false, -R(20, 180), progress),
          ),
        );
      }
      requestAnimationFrame(drawPile);
